...
class CalendarMCPServer:
    ...
    async def run(self):
        ...
        if self.config.server_type == "http":
            # Start the FastAPI-based HTTP server which exposes /sse, /list_tools and /call_tool
            from http_server import FastAPIMCPServer
            http_server = FastAPIMCPServer(
                self.server,
                host=self.config.server_host,
                port=self.config.server_port
            )
            logger.info(f"Starting FastAPI MCP Server on {self.config.server_host}:{self.config.server_port}")
            try:
    ...
    async def list_tools_handler(self) -> List[types.Tool]:
        """Return the same tools list as the MCP list_tools handler (for HTTP endpoints)."""
        logger.info("HTTP request: list_tools")
        tools = [
            types.Tool(
                name="query_calendar",
                description="Query Google Calendar for appointments and events using natural language",
                inputSchema={
                    ...
                    "properties": {
                        "query": {"type": "string"},
                        "timezone": {"type": "string", "default": None},
        ...
        logger.debug(f"HTTP list_tools response: {json.dumps([t.__dict__ for t in tools], default=str)}")
        return tools

    async def call_tool_handler(self, name: str, arguments: Dict[str, Any]):
        """Invoke the corresponding tool handler for HTTP call_tool requests."""
        logger.info(f"HTTP call_tool: name={name}, arguments={arguments}")
        try:
            if name == "query_calendar":
                response = await self._handle_calendar_query(arguments)
            elif name == "get_upcoming_events":
                ...
                response = [types.TextContent(type="text", text=f"Unknown tool: {name}")]

            logger.debug(f"HTTP call_tool response: {json.dumps([r.__dict__ for r in response], default=str)}")
            return response
        except Exception as e:
            logger.exception("Error in HTTP call_tool_handler")
